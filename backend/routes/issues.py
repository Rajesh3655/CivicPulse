...
def save_temp_upload(file_storage, prefix):
    filename = secure_filename(f"{prefix}_{datetime.now().timestamp()}_{file_storage.filename}")
...
def build_public_upload_url(file_ref):
    if not file_ref:
        return None

    value = str(file_ref).replace('\\', '/').strip()
    if not value:
        return None
    ...
    if value.startswith('uploads/'):
        ...
        relative_path = f"uploads/{value.split('/')[-1]}"

    if has_request_context():
        return request.url_root.rstrip('/') + '/' + relative_path
...
def serialize_issue(issue):
    image_url = issue.get('image_url') or build_public_upload_url(issue.get('image'))
    resolution_image_url = issue.get('resolution_image_url') or build_public_upload_url(issue.get('resolution_image'))
    return {
        'id': str(issue['_id']),
        'title': issue['title'],
...
@issues_bp.route('/debug/latest', methods=['GET'])
def get_latest_issue_debug():
    db = current_app.db
    user_id = session.get('user_id')
    role = session.get('role')
    if not user_id:
    ...
    issue = db.issues.find_one(query, sort=[('created_at', -1)])
    if not issue:
    ...
    return jsonify({
        'issue': serialized,
        'debug': {
            'raw_image': issue.get('image'),
            'raw_image_url': issue.get('image_url'),
            'raw_resolution_image': issue.get('resolution_image'),
            'raw_resolution_image_url': issue.get('resolution_image_url')
        }
    }), 200
...
def ai_preview():
    ...
    if is_multipart and 'image' in request.files:
        ...
        if file and file.filename and allowed_file(file.filename):
            _, preview_image_path = save_temp_upload(file, 'preview')
...
def create_issue():
    ...
    try:
        ...
        if 'image' in request.files:
            file = request.files['image']
            if file and file.filename and allowed_file(file.filename):
                image_filename, image_temp_path = save_temp_upload(file, 'issue')

        # Get user info
        user = db.users.find_one({'_id': ObjectId(user_id)})
        user_name = user['name'] if user else 'Unknown'

        ai_result = analyze_issue(db, title, description, lat, lon, image_path=image_temp_path)

        duplicate_distance = ai_result.get('duplicate_distance_meters')
        if (
            not confirm_not_duplicate
            and ai_result.get('duplicate_issue_id')
        ...
        if image_temp_path:
            upload_result = upload_image(
                ...
                public_id=f"issue_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
            )
            cloudinary_image = upload_result.get('secure_url') or upload_result.get('url') or image_filename
        ...
        return jsonify({
            ...
            'debug_image_url': issue.get('image'),
            'debug_resolution_image_url': issue.get('resolution_image')
        }), 201
    except RuntimeError as exc:
        return jsonify({'error': str(exc)}), 500
    except Exception as exc:
        return jsonify({'error': f'Image upload failed: {str(exc)}'}), 500
    finally:
        cleanup_temp_file(image_temp_path)
...
def update_status(issue_id):
    ...
    resolution_image = None
    resolution_temp_path = None
    if hasattr(request, 'files') and 'resolution_image' in request.files:
        file = request.files['resolution_image']
        if file and file.filename and allowed_file(file.filename):
            filename, resolution_temp_path = save_temp_upload(file, 'resolution')
            upload_result = upload_image(
                resolution_temp_path,
                folder='civicpulse/resolutions',
                public_id=f"resolution_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}"
            )
            resolution_image = upload_result.get('secure_url') or upload_result.get('url') or filename
    ...
    try:
        ...
        if resolution_image:
            ...
            update_data['resolution_image_url'] = resolution_image
        ...
        cleanup_temp_file(resolution_temp_path)
